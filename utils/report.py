"""
Report Generator — CSV and PDF export of full analysis results
"""
import io
import re
import pandas as pd
...
def _clean_pdf(text: str) -> str:
    """Strip HTML tags and non-ASCII chars (emojis) — fpdf only supports Latin."""
...
def generate_report_pdf(
    ...
    W = pdf.w - pdf.l_margin - pdf.r_margin  # 170mm usable width

    def section_title(text: str):
        pdf.ln(4)
        pdf.set_font("Arial", "B", 13)
        pdf.set_fill_color(240, 242, 255)
        pdf.cell(W, 8, _clean_pdf(text), ln=True, fill=True)
        pdf.ln(2)

    def row_line(label: str, value: str, bold_label: bool = False):
        pdf.set_font("Arial", "B" if bold_label else "", 10)
        pdf.cell(70, 7, _clean_pdf(label), ln=False)
        pdf.set_font("Arial", "", 10)
        pdf.cell(W - 70, 7, _clean_pdf(value), ln=True)
    ...
    if target_result:
        ...
        pdf.cell(W, 8, _clean_pdf(tr["label"]), ln=True)
        pdf.set_font("Arial", "", 10)
        pdf.ln(2)
        for label, value in [
            ...
            ("Check Period",      _clean_pdf(tr["period"])),
        ]:
            row_line(label, value)
        pdf.ln(2)
        pdf.set_font("Arial", "I", 10)
        pdf.multi_cell(W, 6, _clean_pdf(tr["note"]))
        pdf.ln(2)
        pdf.set_font("Arial", "B", 10)
        pdf.cell(45, 7, "Recommendation:", ln=False)
        pdf.set_font("Arial", "", 10)
        pdf.multi_cell(W - 45, 7, _clean_pdf(tr["recommendation"]))
    ...
    if models:
        ...
        for h, w in zip(["Model", "RMSE ($)", "MAE ($)", "R2", "CV RMSE"], col_w):
        ...
        for m in models:
            ...
            for val, w in zip(
                [m['name'], f"${m['rmse']:,.0f}", f"${m['mae']:,.0f}",
                 f"{m['r2']:.4f}", cv], col_w
            ):
                pdf.cell(w, 7, _clean_pdf(val), border=1,
                         ln=False, align="C", fill=is_best)
            pdf.ln()
            if is_best:
    ...
    if feature_importance is not None:
        ...
        for _, fi_row in feature_importance.head(8).iterrows():
            pdf.cell(100, 7, _clean_pdf(str(fi_row['Feature'])), border=1, ln=False)
            pdf.cell(W - 100, 7, f"{fi_row['Importance']:.1f}%",
    ...
    # fpdf2 — output() returns bytes directly, no wrapper needed
    return pdf.output()
